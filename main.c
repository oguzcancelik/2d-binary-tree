...
node *insert(node *tree, int data1, int data2, int depth){  //insert fuction
	if(tree==NULL){
...
int maxDepth(node *tree){  //find maximum depth
...
void print(node *tree ){ //print tree
	if(tree == NULL){
		return;
	}
...
void printAll(node *tree){ //print tree with depth levels
...
node *maxKey1(node *tree){ //find maximum key1 value
...
node *maxKey2(node *tree){ //find maximum key2 value
...
node *minKey1(node *tree){ //find minimum key1 value
...
node *minKey2(node *tree){ //find minimum key2 value
...
node *search(node *tree, int data1, int data2){ //search for the node to delete
...
node *find(node *tree, int data, int check){ //find if a number is in the tree or not
...
node *delete(node *tree, int data1, int data2){ //delete function
	if(tree == NULL){
		return NULL;
	}
	...
	if(tree->key1 == data1 && tree->key2 == data2){
		...
		if(tree->dph %2==0){
			if(tree->right != NULL){
				...
				tree->right = delete(tree->right, tree->key1, tree->key2);
				return tree;
			}
			tree->key1 = maxKey1(tree->left)->key1;
			tree->key2 = maxKey1(tree->left)->key2;
			tree->left = delete(tree->left, tree->key1, tree->key2);
			return tree;
		}
		if(tree->right != NULL){
	...
	if(tree->dph %2==0){
		if(tree->key1 < data1){
			tree->right = delete(tree->right, data1, data2);
			return tree;
		}
		tree->left = delete(tree->left, data1, data2);
		return tree;
	}
	if(tree->key2 < data2){
		tree->right = delete(tree->right, data1, data2);
